...
/**
 * Alter the post display of the Portfolio custom post type, so thatit display posts as the order the 
 * a rrangemnt rathern than ID
 *
 * @global string $vars
 */
function custom_archive_portfolio_order( $vars ) {

  if ( !is_admin() && isset($vars['post_type']) && $vars['post_type'] == 'portfolio' ) {
    $vars['orderby']    = 'menu_order';
    $vars['order']		= 'ASC'; // that's the default behaviour
  }

  return $vars;
}
add_filter( 'request', 'custom_archive_portfolio_order');